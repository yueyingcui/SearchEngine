 ...
 *		* 0: succeed
*/
int InvertedIndex::parse_directory(const std::string directory){
	DIR * dir = NULL; 
	...
	if(!dir){
		...
		if(!entry){
			...
			while((entry = readdir(dir)) != NULL){
				// debug info
				std::cout << "indexing: http://s3.amazonaws.com/shoefitr-recruit/review-indexer/" << entry -> d_name << std::endl;

				std::string file = directory + "/" + entry -> d_name;
				int r;
				r = this -> parse_file(file);
				if(r < 0){
					std::cout << "parse file: " << file << "failed" <<std::endl;
					return -1;
				}
			}
		}
	...
	while(getline(fin, line)){	
		...
		while(stream >> word){
			...
			while(string_iter != word.end()){
				...
				// remove punctuation
				if((*(string_iter) < 'A' || *(string_iter) > 'Z') && (*(string_iter) < 'a' || *(string_iter) > 'z') && (*(string_iter) != '\'')){
